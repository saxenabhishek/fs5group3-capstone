...
public class ClientService {
	...
	public void addPreference(Preference pre) {
		if(pre==null) {
	...
	public void updatePreference(String purpose,String risk,String category,String length) {
		if (purpose==null || risk==null || category==null || length==null) {
		...
		for (Preference pre:preferences) {
			...
			if(!pre.getRiskTolerance().equals(risk)) {
				if(risk.length()>1) {
					pre.setRiskTolerance(risk);
				}
				
			}
			if(!pre.getIncomeCategory().equals(category)) {
				if(category.length()>1) {
					pre.setIncomeCategory(category);
				}
				
			}
			if(!pre.getLengthOfInvestmet().equals(length)) {
				if(length.length()>1) {
